...
SELECT lead_studio, COUNT(*) AS users FROM movies_library.movies INNER JOIN movies_library.users on movie_id = favorite_movie_id GROUP BY lead_studio ORDER BY users desc LIMIT 1;

-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 5 >>>>>>>>>>>>>>>>>>>>>>>
-- Get the average Rotten Tomatoes score of all movies that are favorited by a user.
-- EXPECTED RESULT: 52.21

SELECT AVG(rotten_tomatoes) FROM movies_library.movies RIGHT JOIN movies_library.users on movie_id = favorite_movie_id;
...
SELECT film, COUNT(*) AS entries FROM movies_library.movies GROUP BY film HAVING entries > 1;
